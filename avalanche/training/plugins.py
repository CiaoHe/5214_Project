...
class GEMPlugin(StrategyPlugin):
    ...
    def __init__(self, patterns_per_experience, memory_strength, 
                only_mlp=False, encoder=None):
        """
        :param patterns_per_experience: number of patterns per experience in the
            memory.
    ...
    def process_input_for_bert(self, mb_x, device):
    ...
    def process_input_for_mlp_only(self, mb_x, device):
        input_ids, attention_mask, token_type_ids = zip(*mb_x)
    ...
    def before_training_iteration(self, strategy, **kwargs):
        ...
        if strategy.training_exp_counter > 0:
            ...
            for t in range(strategy.training_exp_counter):
                strategy.optimizer.zero_grad()
                if self.only_mlp:
                    xref = self.process_input_for_mlp_only(self.memory_x[t], strategy.device)
                else:
                    xref = self.process_input_for_bert(self.memory_x[t], strategy.device)
...
class EWCPlugin(StrategyPlugin):
    ...
    def __init__(self, ewc_lambda, mode='separate', decay_factor=None,
                 keep_importance_data=False, only_mlp=False, encoder=None):
        """
    ...
    def compute_importances(self, model, criterion, optimizer,
        ...
        for i, (x, y, _) in enumerate(dataloader):
            ...
            if self.only_mlp:
                x = self.process_input_for_mlp_only(x, device)
            else:
                x = self.process_input_for_bert(x, device)
            y = y.to(device)
