...
class Replay(BaseStrategy):
    ...
    def __init__(self, model: Module, optimizer: Optimizer, criterion,
                 ...
                 eval_mb_size: int = None, device=None,
                 plugins: Optional[List[StrategyPlugin]] = None,
                 evaluator: EvaluationPlugin = default_logger, eval_every=-1, only_mlp=False):
        ...
        super().__init__(
            ...
            train_mb_size=train_mb_size, train_epochs=train_epochs,
            eval_mb_size=eval_mb_size, device=device, plugins=plugins,
            evaluator=evaluator, eval_every=eval_every, only_mlp=only_mlp)
...
class GEM(BaseStrategy):
    ...
    def __init__(self, model: Module, optimizer: Optimizer, criterion,
        ...
        gem = GEMPlugin(patterns_per_exp, memory_strength, only_mlp, encoder)
        if plugins is None:
            plugins = [gem]
        else:
            plugins.append(gem)
        ...
        super().__init__(
            ...
            train_mb_size=train_mb_size, train_epochs=train_epochs,
            eval_mb_size=eval_mb_size, device=device, plugins=plugins,
            evaluator=evaluator, eval_every=eval_every, only_mlp=only_mlp, encoder=encoder)
...
class EWC(BaseStrategy):
    ...
    def __init__(self, model: Module, optimizer: Optimizer, criterion,
        ...
        ewc = EWCPlugin(ewc_lambda, mode, decay_factor, keep_importance_data, only_mlp, encoder)
        if plugins is None:
            plugins = [ewc]
        else:
            plugins.append(ewc)
